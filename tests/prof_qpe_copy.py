"""量化 pre-MLA fragment op 在 graph 路径下的成本（差分法）。

当前 kernel 已把 cos/sin gather + QpeOut 直写移进 kernel，故 fragment op 主要
剩余项是 q_pe 的外部拷贝路径（对照用）。tag:
  full   : 用 kernel 输出的紧凑 QpeOut（= 现在线上路径，无外部拷贝）
  copy   : 忽略 QpeOut，外部 q_out_p[:,:,0,qk_nope:].contiguous()（旧路径，量拷贝成本）
...
def attn_qpe(self, x_normed, block, layer_idx, bs, graph, cm, block_table):
    ...
    k_pers, q_out_p, q_pe_kern = get_premla_persistent_kernel(
    ...
    A_in = k_pers(attn._q_w, attn._q_b, cos, sin, attn._kva_w, attn._kva_b,
                  ...
                  x16, q_out_p, q_pe_kern, bt, new_pos.to(torch.int32), k_cache, v_cache)
    A_in = A_in.reshape(bs, self._num_heads, self._kv_lora_rank)

    if TAG == "full":
        q_pe = q_pe_kern                          # kernel 直写的紧凑 buffer（无线上拷贝）
    else:  # copy：旧路径，外部 slice+contiguous
...
adp.DeepSeekAdapter.attention = attn_qpe
