...
def test_alternative_format():
    ...
    # 模拟替代格式的参数
    M, K, N = 8, 4096, 12288  # 输入维度4096，输出维度12288
    groupsize = 128
    ...
    # 替代格式：qweight [N, N//8]
    qweight = torch.randint(0, 256, (N, N // 8), dtype=torch.int32, device='cuda')
    
    # 替代格式：scales [num_groups, K]
    num_groups = K // groupsize
    scales = torch.randn(num_groups, K, dtype=torch.float16, device='cuda')
    
    # 替代格式：qzeros [num_groups, N//8]
    qzeros = torch.randint(0, 16, (num_groups, N // 8), dtype=torch.int32, device='cuda')
    
    print(f"输入形状: {input_tensor.shape}")
    print(f"qweight形状: {qweight.shape} (替代格式: [N, N//8])")
    print(f"scales形状: {scales.shape} (标准格式: [num_groups, K])")
    print(f"qzeros形状: {qzeros.shape} (替代格式: [num_groups, N//8])")
