...
def test_qwen7b_output_projection_format():
    """测试Qwen7B输出投影格式检测"""
    print("🧪 测试Qwen7B输出投影格式检测...")
    ...
    # 实际参数: qweight=[512, 4096], qzeros=[32, 512], scales=[32, 4096]
    ...
    # 验证格式检测条件
    N = qweight.shape[0]  # 512
    input_K = input_tensor.shape[1]  # 4096
    qweight_cols = qweight.shape[1]  # 4096
    qzeros_cols = qzeros.shape[1]  # 512
    scales_cols = scales.shape[1]  # 4096
    
    print(f"格式检测条件:")
    print(f"  N (qweight.shape[0]): {N}")
    print(f"  input_K: {input_K}")
    print(f"  qweight_cols: {qweight_cols}")
    print(f"  qzeros_cols: {qzeros_cols}")
    ...
    if condition1:
        print(f"✅ Qwen7B输出投影格式检测条件满足")
        return True
    else:
        print(f"❌ Qwen7B输出投影格式检测条件不满足")
        return False

def test_qwen7b_output_projection_processing():
    """测试Qwen7B输出投影处理"""
    print("\n🧪 测试Qwen7B输出投影处理...")
...
def main():
    ...
    tests = [
        ("格式检测测试", test_qwen7b_output_projection_format),
        ("GPTQ处理测试", test_qwen7b_output_projection_processing)
    ]
    ...
    if passed == total:
        print("🎉 所有Qwen7B输出投影格式检测修复测试通过！")
    else:
        print("⚠️  部分Qwen7B输出投影格式检测修复测试失败")
