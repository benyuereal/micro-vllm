        ...
        if ws > 1:
            if rank0():
                ...
                if bt == "waiting" or not b:
                    _t0(); eng.tp_broadcast_waiting(done); _t1("bcast1")
                else:
                    _t0(); ctx = BatchInferenceContext(len(b), bt, b); eng.tp_broadcast_batch(ctx, done); _t1("bcast1")
                ...
                _t0(); ctx, done = eng.tp_receive_batch(); _t1("bcast1r")
