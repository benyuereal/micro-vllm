...
__global__ void fused_ln_qkv_gptq_kernel(
  ...
  float scales_qkv[4];
  for (int i = 0; i < 4; i++) {
    scales_qkv[i] = __half2float(scales[group * hidden_dim + n + i]);
  }
...
#pragma unroll
  ...
  if (n < hidden_dim * 3) {  // 确保不超出QKV总维度
    for (int m = 0; m < m_count; m++) {
      ...
      if (seq_idx < batch_size * seq_len) {
        // 计算在QKV输出中的正确位置
        int qkv_start = seq_idx * hidden_dim * 3;
        int col_start = n;
        
        // 写入Q、K、V部分
        if (col_start < hidden_dim) {
          // Q部分
          qkv_output[qkv_start + col_start] = __float2half_rn(block_c[m][0]);
        } else if (col_start < hidden_dim * 2) {
          // K部分
          qkv_output[qkv_start + col_start] = __float2half_rn(block_c[m][1]);
        } else {
          // V部分
          qkv_output[qkv_start + col_start] = __float2half_rn(block_c[m][2]);
        }
      }
    }
  }
...
torch::Tensor fused_ln_qkv_gptq_cuda(
    ...
    auto v = qkv_output.slice(1, hidden_dim*2, hidden_dim*3).view({batch_size, seq_len, hidden_dim});
