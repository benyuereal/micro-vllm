/*
 * Marlin 权重 repack（GPTQ packed int8 [K/4,N] → Marlin 布局）。
 * 从 vLLM csrc/libtorch_stable/quantization/marlin/gptq_marlin_repack.cu 移植，
 * stable-ABI 类型换成标准 torch/extension.h。
 */
...
torch::Tensor gptq_marlin_repack(torch::Tensor& b_q_weight,
  ...
  TORCH_CHECK(
      b_q_weight.scalar_type() == torch::kInt,
      "b_q_weight type is not kInt");

  TORCH_CHECK(perm.is_cuda(), "perm is not on GPU");
  TORCH_CHECK(perm.is_contiguous(), "perm is not contiguous");
  TORCH_CHECK(perm.scalar_type() == torch::kInt,
              "perm type is not at::kInt");

  const int32_t device_index = b_q_weight.get_device();
  ...
  torch::Tensor out = torch::empty(
      ...
      torch::TensorOptions().dtype(b_q_weight.scalar_type())
