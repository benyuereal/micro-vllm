 ...
 * int32 accumulate，shared mem 只存 int8）。从 vLLM csrc/libtorch_stable/quantization/
 * marlin/marlin.cu 移植，stable-ABI 类型换成标准 torch/extension.h（micro-vllm 编译体系）。
 *
 * 只保留 int8 权重 (kU8B128) + bf16/fp16 激活 + group-128 路径（见 generate_kernels.py）。
 * 用法（Python 侧 kernel/marlin/__init__.py）：
 *   wq = gptq_marlin_repack(wq_packed[K/4,N], perm, size_k, size_n, 8, False)
 *   out = marlin_gemm(x, wq, ws, zp, g_idx, perm, workspace, size_m, size_n, size_k,
...
bool is_valid_config(thread_config_t const& th_config, int thread_m_blocks,
                     int prob_m, int prob_n, int prob_k, int num_bits,
                     int group_size, bool has_act_order, bool is_k_full,
                     int has_zp, bool is_zp_float, int is_a_8bit, int stages,
                     int max_shared_mem) {
  // Sanity
...
torch::Tensor marlin_gemm(
  ...
  if (a.scalar_type() == torch::kHalf) {
    ...
    if (b_scales.scalar_type() == torch::kHalf) {
      c_type_id = vllm::kFloat16.id();
    } else if (b_scales.scalar_type() == torch::kBFloat16) {
      c_type_id = vllm::kBFloat16.id();
    } else {
      c_type_id = vllm::kBFloat16.id();
      ...
      if (c.scalar_type() == torch::kHalf) {
        c_type_id = vllm::kFloat16.id();
      } else if (c.scalar_type() == torch::kBFloat16) {
        c_type_id = vllm::kBFloat16.id();
      } else {
    ...
    } else if (a.scalar_type() == torch::kChar) {
  ...
  if (b_type_id == vllm::kFE2M1f.id()) {
    if (b_scales.scalar_type() == torch::kFloat8_e4m3fn) {
      s_type_id = vllm::kFE4M3fn.id();
    } else if (b_scales.scalar_type() == torch::kFloat8_e8m0fnu) {
      ...
      TORCH_CHECK(
          ...
          "float8_e4m3fn (for NVFP4) or float8_e8m0fnu (for MXFP4).");
             ...
             b_scales.scalar_type() == torch::kFloat8_e8m0fnu) {
    s_type_id = vllm::kFE8M0fnu.id();
  }
  ...
  TORCH_CHECK(a.is_cuda(), "A is not on GPU");
  ...
  if (a_scales_or_none.has_value()) {
    ...
    a_scales = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat).device(device));
    TORCH_CHECK(
        a_type.size_bits() != 8,
  ...
  int sms = -1;
  const int32_t device_index = a.get_device();
  ...
  if (c_or_none.has_value()) {
    c = c_or_none.value();
    TORCH_CHECK(c.is_cuda(), "c is not on GPU");
    TORCH_CHECK(c.is_contiguous(), "c is not contiguous");
    TORCH_CHECK(c.size(0) == size_m,
                "Shape mismatch: c.size(0) = ", c.size(0),
                ", size_m = ", size_m);
    TORCH_CHECK(c.size(1) == size_n,
                "Shape mismatch: c.size(1) = ", c.size(1),
                ", size_n = ", size_n);
  } else {
    c = torch::empty({size_m, size_n}, torch::TensorOptions().dtype(c_scalar_type).device(device));
  }
  ...
  if (use_fp32_reduce) {
    ...
    c_tmp = torch::empty({max_c_tmp_size}, torch::TensorOptions().dtype(torch::kFloat).device(device));
  } else {
    c_tmp = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat).device(device));
  }
  ...
  if (g_idx_or_none.has_value() && perm_or_none.has_value()) {
    ...
    TORCH_CHECK(g_idx.is_cuda(), "g_idx is not on GPU");
    ...
    g_idx = torch::empty({0}, torch::TensorOptions().dtype(c_scalar_type).device(device));
    perm = torch::empty({0}, torch::TensorOptions().dtype(c_scalar_type).device(device));
    a_tmp = torch::empty({0}, torch::TensorOptions().dtype(c_scalar_type).device(device));
  }
  bool has_act_order = g_idx.size(-1) > 0 && perm.size(-1) > 0;

  if (has_act_order) {
    a_tmp = torch::empty({size_m, size_k}, torch::TensorOptions().dtype(c_scalar_type).device(device));
    if (is_k_full) {
  ...
  if (global_scale_or_none.has_value()) {
    ...
    global_scale = torch::empty(
        {0}, torch::TensorOptions().dtype(torch::kFloat).device(device));
    TORCH_CHECK(
  ...
  if (has_bias) {
    ...
    TORCH_CHECK(b_bias.is_cuda(), "b_bias is not on GPU");
    ...
    b_bias = torch::empty({0}, torch::TensorOptions().dtype(c_scalar_type).device(device));
  }

  torch::Tensor b_zeros;
  if (b_zeros_or_none.has_value()) {
    b_zeros = b_zeros_or_none.value();
    TORCH_CHECK(b_zeros.is_cuda(), "b_zeros is not on GPU");
    TORCH_CHECK(b_zeros.is_contiguous(), "b_zeros is not contiguous");
  } else {
    b_zeros = torch::empty({0}, torch::TensorOptions().dtype(c_scalar_type).device(device));
  }
  ...
  TORCH_CHECK(
      a_scales.scalar_type() == torch::kFloat,
      "scalar type of a_scales must be float");
  TORCH_CHECK(
      global_scale.scalar_type() == torch::kFloat,
  ...
  marlin::marlin_mm(
      ...
      c10::cuda::getCurrentCUDAStream(device_index), thread_k, thread_n, sms,
      use_atomic_add, use_fp32_reduce, is_zp_float);

  return c;
}

// 返回 int8 权重 (kU8B128) 的 ScalarType id，供 Python 侧 marlin_gemm 传 b_type_id。
int64_t marlin_u8b128_id() { return vllm::kU8B128.id(); }

// gptq_marlin_repack 定义在 gptq_marlin_repack.cu（同 extension），此处声明供绑定。
torch::Tensor gptq_marlin_repack(torch::Tensor& b_q_weight, torch::Tensor& perm,
                                 int64_t size_k, int64_t size_n, int64_t num_bits,
                                 bool is_a_8bit);

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("marlin_gemm", &marlin_gemm,
        "Marlin W8A16 int8 GEMM (bf16 act, int8 weight, group-128)");
  m.def("gptq_marlin_repack", &gptq_marlin_repack,
        "Marlin weight repack (GPTQ packed int8 -> Marlin layout)");
  m.def("marlin_u8b128_id", &marlin_u8b128_id,
        "kU8B128 ScalarType id for marlin_gemm b_type_id");
}
