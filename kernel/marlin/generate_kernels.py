# 生成 Marlin int8 (kU8B128) + bf16 激活的 kernel 实例化 + kernel_selector.h。
# 从 vLLM generate_kernels.py 裁剪：只保留 GPTQ-INT8 (W8A16) 配置，
# 大幅缩短编译时间（我们只需 int8 权重 + bf16 激活 + group-128）。
import os
import itertools
import jinja2
...
# 只保留 GPTQ-INT8 (W8A16)：bf16 激活 + int8 权重 + group-128
QUANT_CONFIGS = [
    {
        "b_type": "kU8B128",
