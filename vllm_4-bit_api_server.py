...
async def startup_event():
    global engine
    model_path = "/root/Qwen-7B-Chat-Int4"
    ...
    try:
        engine_args = AsyncEngineArgs(
            ...
            quantization="gptq",  # 添加量化方法指定
            dtype="auto",  # 自动检测数据类型
            load_format="auto",  # 自动选择加载格式
            enforce_eager=True,  # 对于量化模型，可能需要启用eager模式
        )
        ...
        print(f"Failed to load 4-bit quantized model: {str(e)}")
        # 尝试回退到标准加载方式
        try:
            ...
            print(f"Fallback loading also failed: {str(fallback_e)}")
            raise e
