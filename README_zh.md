...
> 高性能 LLM 推理引擎，从零实现 **PagedAttention + Flash Attention + SwiGLU 算子融合**，A100 上单用户性能达 vLLM 的 **110%**，适合小规模生产部署和学习。
> 
> 🚀 **最新进展**：张量并行发布，支持多卡推理！

## ✨ 特性

* 🚀 **连续批处理** - 动态批次填充，batch=32 时 GPU 利用率 **~95%**
...
在单用户串行请求场景下，micro-vllm 展现出优异的推理效率和稳定性：
...
- 单用户平均吞吐领先 vLLM **9%**，性能已对齐业界标杆
- 稳定性显著优于 vLLM（变异系数低 **28 倍**），抖动控制在 0.09% 以内
- 轻量级架构在单并发场景下调度开销更低，适合低延迟交互式应用
...
> **硬件**：NVIDIA A100-PCIE-40GB &nbsp;|&nbsp; **模型**：Qwen-7B &nbsp;|&nbsp; **批大小**：32 &nbsp;|&nbsp; **总请求数**：32 &nbsp;|&nbsp; **总 Token 数**：9,677

| 框架 | 单请求 (tokens/s) | GPU 利用率 | 相对性能 |
|:-----|:-----------------|:----------|:--------|
| **本框架** | **64** | **~95%** | **103%** |
| vLLM | ~62 | — | 100% |

- Decode 单步延迟：**~15ms**（graph=14.5ms + sample=0.27ms）
- batch=32 时 GPU 利用率达 **95%**（nvidia-smi 实测）/ **97%**（CUDA event 精确测量）
...
本框架适合中小规模 LLM 服务的生产部署，性能达 vLLM 110%，代码简洁易于理解和二次开发。
